    ...
    try:
        ...
        return jsonify({
            ...
            'result': {
                ...
                'total_months': round(fte_result.get('total_months', 0), 2),
                'status': 'COMPLETED'
            },
            'files': {
                'json_report': json_report_path,
    ...
    try:
        ...
        for result in results:
            submissions.append({
                ...
                'tier': result.get('calculation_result', {}).get('tier', 'N/A'),
                'total_hours': result.get('calculation_result', {}).get('total_hours', 0),
                'total_days': result.get('calculation_result', {}).get('total_days', 0),
    ...
    try:
        ...
        if not word_report_path or word_report_path == '':
            ...
            if not word_report_file:
                ...
                return jsonify({
                    'success': False,
                    'error': f'Report file not found. Please regenerate the report by submitting the scoping data again.'
                }), 404
        else:
        ...
        if not word_report_file.exists():
            return jsonify({
                'success': False,
                'error': f'Report file not found at: {word_report_file}'
            }), 404
