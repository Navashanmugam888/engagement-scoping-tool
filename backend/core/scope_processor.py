...
from backend.config import EXCEL_FILE, SHEET_SCOPE_DEFINITION, AVAILABLE_ROLES, TIERS
...
class ScopeDefinitionProcessor:
    ...
    def _load_metrics(self):
        """Load all metrics from Scope Definition sheet"""
        wb = load_workbook(EXCEL_FILE, data_only=True)
        ws = wb[SHEET_SCOPE_DEFINITION]
        
        # Section headings to skip
        section_headings = [
            'Dimensions', 'Application Features', 'Application Customization',
            'Calculations', 'Security', 'Historical Data', 'Integrations',
            'Data Migration', 'Other Requirements'
        ]
        
        for row in range(6, 103):
            metric_name = ws.cell(row=row, column=2).value  # Column B
            if metric_name and isinstance(metric_name, str) and metric_name.strip():
                stripped_name = metric_name.strip()
                
                # Skip section headings and items without leading spaces
                if stripped_name in section_headings or not metric_name.startswith(' '):
                    continue
                
                # Check if Details cell is grey/locked
                details_cell = ws.cell(row=row, column=4)
                is_grey = details_cell.fill.start_color.index != '00000000' if details_cell.fill else False
                ...
                self.metrics.append({
                    ...
                    'details': None,
                    'weightage': 0,
                    'feature_clean': stripped_name,
                    'in_scope_flag': 0,
                    'is_details_required': not is_grey,
                    'is_sub_question': metric_name.startswith('          ')
                })
